...
<!-- 
	...
	1. Application 객체 (전역 자원) >> 접속하는 모든 사용자가 다룰 수 있는 자원
	
	-사이트에 접속하는 모든 session이 가지고 노는 객체
	-ex) 전체 사이트 접속자 수
	-application.setAttribute("total", 0): total모든 사용자가 다룰 수 있는 전역자원
	-사이트 접속시 total += 1;
	
	2. session 객체(고유한 자원) 	>> 접속하는 브라우저 마다 (sessionID)	고유하게 부여되는 객체(자원)
	-ex) 접속한 사용자 마다
